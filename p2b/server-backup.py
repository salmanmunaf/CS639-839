...
def new_block_received():
    ...
    if block.number == 1:
        ...
        # blockchain.current_transactions = blockchain.state.get_invalid_transactions(blockchain.current_transactions, block.transactions)
        # self.current_transactions = self.state.get_invalid_transactions(self.current_transactions, valid_txns)
    # print("here2222222")

    # TODO - done: if I am responsible for next block, start mining it (trigger_new_block_mine).
    if blockchain.node_identifier == blockchain.get_next_miner(block.miner):
